...
than KRACH; this one specifically tests the property KRACH is expected
to FAIL and NPI (bounded 0-100 scale) is expected to PASS: as a
Bradley-Terry MLE, KRACH has no finite optimum for a perfectly
undefeated or perfectly winless team (a "perfect separator" in the
comparison graph) -- the true MLE rating diverges to infinity/zero.
`src/rankings/krach.py` has a `max(points, 0.1)` floor that prevents an
immediate divide-by-zero on the first iteration, but nothing stops the
iterative solver from driving toward an extreme value over its 1000
iterations, and its per-iteration renormalization
(`ratings / ratings.mean() * 100`) means one extreme team's rating could
distort the WHOLE field's ratings, not just its own -- tested directly
here, not assumed.

Design: simulate a normal season, then force one designated team to win
EVERY one of its games (regardless of what actually happened) and a
second to lose every one of its games, holding every other game
unchanged. Fit KRACH, NPI, Massey, and RPI on both the baseline and the
forced season. Measure (a) the extreme team's own rating -- is it
finite and reasonably scaled, or does it blow up / collapse? -- and (b)
collateral distortion: how much do the OTHER, unrelated teams' ranks
shift between baseline and forced season, as a measure of whether one
degenerate team destabilizes the whole field, not just itself.

Run from the project root as a module:
    python -m research.npi_critique.experiments.e11_connectivity_degenerate_cases
"""
import numpy as np
import pandas as pd
...
from research.npi_critique.harness.simulate import assign_true_strengths, simulate_season, games_played
...
def main():
    ...
    for rep in range(N_REPLICATIONS):
        ...
        sim_baseline = simulate_season(schedule, strengths, rng)
    ...
    extreme_df.to_csv(results_path("e11_connectivity_degenerate_cases", "extreme_team_ratings.csv"), index=False)
    collateral_df.to_csv(results_path("e11_connectivity_degenerate_cases", "collateral_distortion.csv"), index=False)
